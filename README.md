# SQL Projects for Data Analysis Upwork Project
...
The project’s Agenda involves Analyzing the data using SQL on the Oracle Database Software. We first download the Oracle Database 21c edition from the Oracle website and understand the problem. Then tables are created in the database followed by data insertion into tables and exploration.
...
b. Correct phone numbers and emails that are not in a proper format and Correct contact number and remove the full name.  
c. Read the BLOB column and fetch attribute details from the regular tag, nested columns.  
...
𝗗𝗮𝘁𝗮 𝗣𝗶𝗽𝗲𝗹𝗶𝗻𝗲:  
System for moving data from one system to another. The data may or may not be transformed, and it may be processed in real-time (or streaming) instead of in batches. A data pipeline is extracting or capturing data using various tools, storing raw data, cleaning, validating data, transforming data into a query-worthy format, visualization KPIs including Orchestration the above process is data pipeline.  
